...
static void __init lsm_prepare(struct lsm_info *lsm)
{
	...
	lsm_blob_size_update(&blobs->lbs_kernfs_node,
			     &blob_sizes.lbs_kernfs_node);
	...
	lsm_blob_size_update(&blobs->lbs_request_sock,
			     &blob_sizes.lbs_request_sock);
	lsm_blob_size_update(&blobs->lbs_skb, &blob_sizes.lbs_skb);
...
int __init security_init(void)
{
	...
	if (lsm_debug) {
		...
		lsm_pr("blob(kernfs_node) size %d\n",
		       blob_sizes.lbs_kernfs_node);
		...
		lsm_pr("blob(sctp_assoc) size %d\n",
		       blob_sizes.lbs_sctp_assoc);
		lsm_pr("blob(request_sock) size %d\n",
		       blob_sizes.lbs_request_sock);
		lsm_pr("blob(skb) size %d\n", blob_sizes.lbs_skb);
