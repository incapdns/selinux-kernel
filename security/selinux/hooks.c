...

{
	...
	if (ssid != tsid_fd) {
		rc = avc_has_perm(cred_selinux_state(cred), ssid, tsid_fd,
			    ...
			    const struct cred_security_struct *new_crsec)
{
	 ...
	 */
	rc = security_bounded_transition(current_selinux_state, old_crsec->sid,
					 new_crsec->sid);
	if (!rc)
		return 0;
...
static int selinux_inode_permission(struct inode *inode, int requested)
{
	...
	if (likely(!rc)) {
		...
		/* Cache miss. */
		rc = avc_has_perm_noaudit(current_selinux_state, sid, isec->sid,
					...
					struct kernfs_node *kn)
{
	...
	if (crsec->create_sid) {
		...
		rc = security_transition_sid(current_selinux_state, crsec->sid,
				 ...
				 u16 secclass, u32 *socksid)
{
	...
	return security_transition_sid(current_selinux_state, crsec->sid,
				       crsec->sid, secclass, NULL, socksid);
}
