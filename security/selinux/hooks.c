			...
			const struct cred *cred)
{
	...
	rc = avc_has_perm(&selinux_state,
			  crsec->sid, sbsec->sid, SECCLASS_FILESYSTEM,
			  FILESYSTEM__RELABELFROM, NULL);
	if (rc)
		return rc;

	rc = avc_has_perm(&selinux_state,
			  crsec->sid, sid, SECCLASS_FILESYSTEM,
...

{
	...
	if (ssid != tsid_fd) {
		rc = avc_has_perm(&selinux_state, ssid, tsid_fd, SECCLASS_FD,
				  FD__USE, &ad);
		if (rc)
			return rc;
	}
				 ...
				 u32 *_new_isid)
{
	...
	if ((sbsec->flags & SE_SBINITIALIZED) &&
		...
		*_new_isid = crsec->create_sid;
		...
		return security_transition_sid(&selinux_state, crsec->sid,
					       dsec->sid, tclass,
					       name, _new_isid);
	}
			    ...
			    const struct cred_security_struct *new_crsec)
{
	 ...
	 */
	rc = security_bounded_transition(&selinux_state, old_crsec->sid,
					 new_crsec->sid);
	if (!rc)
		return 0;
...
static int selinux_bprm_creds_for_exec(struct linux_binprm *bprm)
{
	...
	if (old_crsec->exec_sid) {
		...
		rc = security_transition_sid(&selinux_state, old_crsec->sid,
	...
	if (new_crsec->sid == old_crsec->sid) {
		rc = avc_has_perm(&selinux_state,
				  old_crsec->sid, isec->sid, isec->sclass,
		...
		rc = avc_has_perm(&selinux_state,
				  new_crsec->sid, isec->sid, isec->sclass,
				  FILE__ENTRYPOINT, &ad);
		if (rc)
			return rc;
					...
					struct kernfs_node *kn)
{
	...
	if (crsec->create_sid) {
		...
		rc = security_transition_sid(&selinux_state, crsec->sid,
				 ...
				 u16 secclass, u32 *socksid)
{
	...
	return security_transition_sid(&selinux_state, crsec->sid, crsec->sid,
				 ...
				 int protocol, int kern)
{
	...
	return avc_has_perm(&selinux_state,
			    crsec->sid, newsid, secclass, SOCKET__CREATE,
			    NULL);
}
...
static int selinux_lsm_setattr(u64 attr, void *value, size_t size)
{
	...
	if (attr == LSM_ATTR_EXEC) {
		...
		/* Check permissions for the transition. */
		error = avc_has_perm(&selinux_state,
				     crsec->sid, sid, SECCLASS_PROCESS,
				  ...
				  struct bpf_token *token, bool kernel)
{
	...
	return avc_has_perm(&selinux_state, ssid, bpfsec->sid, SECCLASS_BPF,
				    ...
				    const struct path *path)
{
	...
	if (bpf_token_cmd(token, BPF_MAP_CREATE)) {
		err = avc_has_perm(&selinux_state, bpfsec->sid, sid,
...
static int selinux_bpf_token_capable(const struct bpf_token *token, int cap)
{
	...
	return avc_has_perm(&selinux_state, current_sid(), bpfsec->grantor_sid,
			    sclass, av, NULL);
}
#endif
