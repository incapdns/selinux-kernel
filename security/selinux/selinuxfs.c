...
enum sel_inos {
	...
	SEL_MAXNSDEPTH,	/* maximum SELinux namespace ancestry depth */
	SEL_NS_CREATE,	/* create a dormant SELinux namespace control FD */
...
static struct selinux_fs_info *selinux_fs_info_create(
	struct selinux_state *state)
{
	...
	fsi->state = get_selinux_state(state);
	return fsi;
}
...
static struct selinux_state *selinux_file_state(const struct file *file)
{
...
/*
 * Charge retained policy snapshots to the policy state, rather than to one
 * selinuxfs superblock. A state can have one superblock per owning user
 * namespace, and opening another mount must not bypass the memory bound.
 */
static int selinuxfs_snapshot_bytes_reserve(
	struct selinux_state *state,
	size_t bytes)
{
...
static void selinuxfs_snapshot_bytes_release(
	struct selinux_state *state,
	size_t bytes)
{
...
static int selinuxfs_has_perm(const struct file *file, u32 permission)
{
	if (selinux_file_state(file) != current_selinux_state)
		return -EXDEV;
...
static int selinuxfs_mutation_allowed(const struct file *file, u32 permission)
{
	struct selinux_state *state = selinux_file_state(file);

	if (state != current_selinux_state)
		return -EXDEV;
	return selinux_state_policy_mutation_allowed(
		state,
		current_cred(),
		permission,
		false);
...
static long sel_ns_create_ioctl(
	struct file *file,
	...
	unsigned long argument)
{
	...
	if (command != SELINUX_NS_IOC_CREATE)
		return -ENOTTY;
	if (argument)
		return -EINVAL;
	if (selinux_file_state(file) != current_selinux_state)
	...
	/* open_namespace() consumes the control reference on every path. */
	return open_namespace(&control->ns);
}

static const struct file_operations sel_ns_create_ops = {
	.unlocked_ioctl = sel_ns_create_ioctl,
				...
				size_t count, loff_t *ppos)
{
	...
	length = scnprintf(
		...
		TMPBUFLEN,
		"%d",
		enforcing_enabled(selinux_file_state(filp)));
...

{
	...
	old_value = enforcing_enabled(selinux_file_state(file));
	if (new_value != old_value) {
		length = selinuxfs_mutation_allowed(
			file,
		...
		enforcing_set(selinux_file_state(file), new_value);
		if (new_value)
			avc_ss_reset(selinux_file_state(file)->avc, 0);
		selnl_notify_setenforce(new_value);
		selinux_status_update_setenforce(
			selinux_file_state(file),
			new_value);
		if (!new_value)
					...
					size_t count, loff_t *ppos)
{
	...
	int handle_unknown = (ino == SEL_REJECT_UNKNOWN) ?
		security_get_reject_unknown(selinux_file_state(filp)) :
		!security_get_allow_unknown(selinux_file_state(filp));
...
static ssize_t sel_read_maxns(
	struct file *file,
	char __user *buffer,
	size_t count,
	loff_t *position)
{
	char value[TMPBUFLEN];
	ssize_t length;
...
static ssize_t sel_write_namespace_limit(
	struct file *file,
	const char __user *buffer,
	size_t count,
	loff_t *position,
	u32 permission,
	int (*set_limit)(unsigned int value))
{
	struct selinux_state *state = selinux_file_state(file);
	unsigned int value;
	char *input;
	...
	/* Namespace-count limits are host-global and owned by the root state. */
...
static ssize_t sel_write_maxns(
	struct file *file,
	const char __user *buffer,
	size_t count,
	loff_t *position)
{
	return sel_write_namespace_limit(
		file,
		buffer,
		count,
		position,
		SECURITY__SETMAXNS,
		selinux_state_set_maxns);
}
...
static ssize_t sel_read_maxnsdepth(
	struct file *file,
	char __user *buffer,
	size_t count,
	loff_t *position)
...
static ssize_t sel_write_maxnsdepth(
	struct file *file,
	const char __user *buffer,
	size_t count,
	loff_t *position)
{
	return sel_write_namespace_limit(
		...
		count,
		position,
		SECURITY__SETMAXNSDEPTH,
		selinux_state_set_maxnsdepth);
}
				...
				size_t count, loff_t *ppos)
{
	...
	length = scnprintf(
		...
		TMPBUFLEN,
		"%d",
		security_mls_enabled(selinux_file_state(filp)));
	return simple_read_from_buffer(buf, count, ppos, tmpbuf, length);
}
...
static int sel_open_policy(struct inode *inode, struct file *filp)
{
	...
	struct selinux_state *state = selinux_file_state(filp);
	int rc;

	rc = selinuxfs_has_perm(filp, SECURITY__READ_POLICY);
	if (rc)
		return rc;
			       ...
			       size_t count, loff_t *ppos)
{
	...
	ret = selinuxfs_has_perm(filp, SECURITY__READ_POLICY);
	if (ret)
		return ret;
...

{
	...
	length = selinuxfs_mutation_allowed(file, SECURITY__LOAD_POLICY);
	if (length)
		return length;
	...
	mutex_lock(&state->policy_mutex);
	length = security_load_policy(state, data, count, &load_state);
	if (length) {
	...
	sha256(data, count, state->policy_digest);
...
static ssize_t sel_write_context(struct file *file, char *buf, size_t size)
{
	...
	length = selinuxfs_has_perm(file, SECURITY__CHECK_CONTEXT);
	if (length)
		goto out;
				     ...
				     size_t count, loff_t *ppos)
{
	...
	length = scnprintf(
		...
		TMPBUFLEN,
		"%u",
		checkreqprot_get(selinux_file_state(filp)));
	return simple_read_from_buffer(buf, count, ppos, tmpbuf, length);
}
					...
					size_t count, loff_t *ppos)
{
	...
	rc = selinuxfs_has_perm(file, SECURITY__VALIDATE_TRANS);
	if (rc)
		goto out;
	...
	rc = security_validate_transition_user(
		state,
		osid,
		nsid,
		tsid,
...
static ssize_t sel_write_access(struct file *file, char *buf, size_t size)
{
	...
	length = selinuxfs_has_perm(file, SECURITY__COMPUTE_AV);
	if (length)
		goto out;
...
static ssize_t sel_write_create(struct file *file, char *buf, size_t size)
{
	...
	length = selinuxfs_has_perm(file, SECURITY__COMPUTE_CREATE);
	if (length)
		goto out;
	...
	length = security_transition_sid_user(
		state,
		ssid,
		tsid,
		tclass,
...
static ssize_t sel_write_relabel(struct file *file, char *buf, size_t size)
{
	...
	length = selinuxfs_has_perm(file, SECURITY__COMPUTE_RELABEL);
	if (length)
		goto out;
...
static ssize_t sel_write_member(struct file *file, char *buf, size_t size)
{
	...
	length = selinuxfs_has_perm(file, SECURITY__COMPUTE_MEMBER);
	if (length)
		goto out;
...
static int selinuxfs_inode_set_label(
	struct selinux_state *leaf,
	struct selinux_policy *leaf_policy,
	struct inode *inode,
	const char *path,
	u16 sclass,
	u32 fallback_sid)
{
	...
	for (state = leaf; state; state = state->parent) {
		...
		if (depth++ > SELINUX_NS_MAX_DEPTH)
			return -E2BIG;
		if (state == leaf && leaf_policy)
			rc = selinux_policy_genfs_sid(
				leaf_policy,
				"selinuxfs",
				path,
				sclass,
			      ...
			      size_t count, loff_t *ppos)
{
	...
	length = selinuxfs_mutation_allowed(filep, SECURITY__SETBOOL);
	if (length)
		goto out;
				      ...
				      size_t count, loff_t *ppos)
{
	...
	if (new_value && fsi->bool_pending_values) {
		length = selinux_chain_update_prepare(fsi->state);
		if (length)
			goto out;
		length = selinux_chain_update_begin(fsi->state);
		if (length) {
			selinux_chain_update_abort(fsi->state);
			goto out;
		}
		length = security_set_bools(
			fsi->state,
			fsi->bool_num,
			fsi->bool_pending_values);
		selinux_chain_update_end(fsi->state);
		selinux_chain_update_complete(fsi->state);
	}
			  ...
			  int **bool_pending_values)
{
	...
	*bool_pending_names = names;
	...
	for (i = 0; !ret && i < num; i++) {
		...
		ret = selinuxfs_inode_set_label(
			((struct selinux_fs_info *)bool_dir->d_sb->s_fs_info)->state,
			newpolicy,
			inode,
			page,
			SECCLASS_FILE,
					    ...
					    size_t count, loff_t *ppos)
{
	...
	length = scnprintf(
		...
		TMPBUFLEN,
		"%u",
		avc_get_cache_threshold(selinux_file_state(filp)->avc));
	return simple_read_from_buffer(buf, count, ppos, tmpbuf, length);
}
...

{
	...
	ret = selinuxfs_mutation_allowed(file, SECURITY__SETSECPARAM);
	if (ret)
		return ret;
	...
	avc_set_cache_threshold(selinux_file_state(file)->avc, new_value);
				       ...
				       size_t count, loff_t *ppos)
{
	...
	length = avc_get_hash_stats(selinux_file_state(filp)->avc, page);
	if (length >= 0)
					...
					size_t count, loff_t *ppos)
{
	...
	length = security_sidtab_hash_stats(selinux_file_state(filp), page);
	if (length >= 0)
				  ...
				  size_t count, loff_t *ppos)
{
	...
	value = security_policycap_supported(
		selinux_file_state(file),
		i_ino & SEL_INO_MASK);
...
static int sel_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	static const struct tree_descr selinux_files[] = {
		...
		[SEL_NS_CREATE] = {"ns_create", &sel_ns_create_ops, S_IRUSR},
		/* last one */ {"", NULL, 0}
	};
...
static int selinuxfs_compare(
	struct super_block *sb,
	struct fs_context *fc)
{
	const struct selinux_fs_info *existing = sb->s_fs_info;
	const struct selinux_fs_info *requested = fc->s_fs_info;

	/*
	 * The state selects the policy authority; the user namespace selects
	 * inode ownership and the VFS capability domain. Both are part of the
	 * identity of a selinuxfs instance.
	 */
	return existing->state == requested->state &&
...
{
	fc->s_fs_info = selinux_fs_info_create(current_selinux_state);
	if (!fc->s_fs_info)
		return -ENOMEM;
...
int __init init_sel_fs(void)
{
	 ...
	 */
	(void)selinux_kernel_status_page(init_selinux_state);
