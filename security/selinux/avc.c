		       ...
		       struct selinux_avc **out)
{
	...
	if (!out)
		return -EINVAL;
	(void)resources;
	avc = kzalloc_obj(*avc, GFP_KERNEL_ACCOUNT);
	if (!avc)
		return -ENOMEM;

	avc->avc_cache_threshold = AVC_DEF_CACHE_THRESHOLD;
	for (i = 0; i < AVC_CACHE_SLOTS; i++) {
		INIT_HLIST_HEAD(&avc->avc_cache.slots[i]);
		spin_lock_init(&avc->avc_cache.slots_lock[i]);
	}
	atomic_set(&avc->avc_cache.active_nodes, 0);
	atomic_set(&avc->avc_cache.lru_hint, 0);
	spin_lock_init(&avc->notif_lock);
	*out = avc;
	return 0;
}

unsigned int avc_get_cache_threshold(const struct selinux_avc *avc)
{
...
static inline struct avc_node *avc_search_node(
 ...
 */
static struct avc_node *avc_lookup(
	struct selinux_avc *avc, u32 ssid, u32 tsid, u16 tclass)
{
	struct avc_node *node;
...
static int avc_latest_notif_update(
	struct selinux_avc *avc, u32 seqno, int is_insert)
{
	int ret = 0;
	unsigned long flag;

	spin_lock_irqsave(&avc->notif_lock, flag);
	if (is_insert) {
	...
	spin_unlock_irqrestore(&avc->notif_lock, flag);
 ...
 */
static void avc_insert(struct selinux_avc *avc,
		       u32 ssid, u32 tsid, u16 tclass,
 ...
 * bigger stack frame.
 */
static noinline void avc_compute_av(struct selinux_state *state,
				    u32 ssid, u32 tsid, u16 tclass,
				    struct av_decision *avd,
				    struct avc_xperms_node *xp_node)
{
 ...
 */
static int avc_has_extended_perms_common(
	...
	bool audit)
{
	...
	if (unlikely(!xpd)) {
		...
		rcu_read_unlock();
		security_compute_xperms_decision(state, ssid, tsid, tclass, driver,
						 base_perm, &local_xpd);
		rcu_read_lock();
		avc_update_node(state->avc, AVC_CALLBACK_ADD_XPERMS,
				requested, driver,
...
decision:
	denied = requested & ~(avd.allowed);
	if (unlikely(denied))
		rc = avc_denied(state, ssid, tsid, tclass, requested, driver,
...
int avc_has_extended_perms_disabled(
	struct selinux_state *state,
	u32 ssid,
	u32 tsid,
	u16 tclass,
	...
	struct common_audit_data *ad)
{
	return avc_has_extended_perms_common(
		state, ssid, tsid, tclass, requested, driver, base_perm,
		xperm, ad, NULL, true);
}

int avc_has_extended_perms_noaudit_disabled(
	struct selinux_state *state,
	u32 ssid,
	u32 tsid,
	u16 tclass,
				    ...
				    struct av_decision *avd)
{
	...
	denied = requested & ~(avd->allowed);
	if (unlikely(denied))
		return avc_denied(state, ssid, tsid, tclass, requested, 0, 0, 0,
				  flags, avd);
	return 0;
}
 ...
 */
inline int avc_has_perm_noaudit_disabled(
	struct selinux_state *state,
	u32 ssid,
	u32 tsid,
	u16 tclass,
	...
	struct common_audit_data *auditdata)
{
	...
	rc = avc_has_perm_noaudit_disabled(
		state,
		ssid,
		tsid,
		tclass,
...
u32 avc_policy_seqno(const struct selinux_state *state)
{
